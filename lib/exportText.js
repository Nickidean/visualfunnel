...
export function exportJourneyText(journey, device) {
  ...
  for (const col of vm.columns) {
    if (col.kind === "step" && col.optional) {
      lines.push(
        `   ↳ (conditional) ${col.step.title}  [${fmtNum(col.through)} went through` +
          `${col.throughShare !== null ? ` (${fmtPct(col.throughShare)})` : ""}` +
          `${
            col.bypass !== null
              ? `, ${fmtNum(col.bypass)} skipped${
                  col.bypassShare !== null ? ` (${fmtPct(col.bypassShare)})` : ""
                }`
              : ""
          }]`
      );
      appendStepDetail(lines, col.step, "      ");
    } else if (col.kind === "step") {
      stepNo += 1;
      lines.push(
