...
export function newLane(name = "Lane") {
  return { id: uid("lane"), name, steps: [newStep({ title: "Step" })] };
}
...
export function newForkSection(laneNames = ["Lane A", "Lane B"]) {
...
export function newJourney(name = "Untitled journey") {
  return {
    ...
    structure: {
      sections: [
        newStepSection(newStep({ title: "Landing" })),
        newStepSection(newStep({ title: "Quote" })),
        newStepSection(newStep({ title: "Confirm" })),
      ],
    },
  };
}
