...
export function deleteStep(structure, stepId) {
  ...
  if (loc.kind === "section") {
    ...
    // Lanes may be left empty (the lane card keeps its "+ step" affordance).
...
export function addStepSection(structure, step) {
  const next = clone(structure);
  next.sections.push(newStepSection(step ? newStep(step) : newStep({ title: "New step" })));
  return next;
}
...
export function addStepToLane(structure, forkId, laneId, step) {
  const next = clone(structure);
  const fork = next.sections.find((s) => s.type === "fork" && s.id === forkId);
  const lane = fork?.lanes.find((l) => l.id === laneId);
  if (lane) lane.steps.push(step ? newStep(step) : newStep({ title: "Step" }));
  return next;
}
...
export function addLane(structure, forkId) {
  ...
  const fork = next.sections.find((s) => s.type === "fork" && s.id === forkId);
  if (fork)
    fork.lanes.push(newLane(`Path ${String.fromCharCode(65 + fork.lanes.length)}`));
  return next;
}
